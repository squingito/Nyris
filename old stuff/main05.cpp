...
int main() {
    ...
    while (1) {
        ...
        DiscriptorWrap* sock = new DiscriptorWrap(clientfd, &peeraddr, nullptr);
        sock->addLayer(new SslProtocolLayer(SERVER_SOCK));
        ...
        while (1) {
            ...
            fdset[1].events |= (sock->dataToWrite() ? POLLOUT : 0);
            ...
            if (fdset[0].revents & POLLIN) {
                ...
                std::vector<char> conversion;
                conversion.insert(conversion.begin(), toPass.c_str(), toPass.c_str() + toPass.size());
                sock->dwrite(&conversion);
            }
